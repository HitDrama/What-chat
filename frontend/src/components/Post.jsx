...
const Post = ({ post }) => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                const updatedPostData = posts.map(p =>
                    p._id === post._id ? {
                        ...p,
                        likes: liked ? p.likes.filter((like) => like !== user._id) : [...p.likes, user._id]
                    } : p
                );
                dipatch(setPosts(updatedPostData));
    ...
    return (
        <div className='my-8 w-full max-w-sm mx-auto'>
            ...
            {
                comment.length > 0 && (
                    <span onClick={() => { dipatch(setSelectedPost(post)); setOpen(true); }} className='cursor-pointer text-sm text-gray-400'>View all {comment.length} comments</span>
                )
            }
